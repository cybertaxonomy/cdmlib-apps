 ...
 */
@Component
public class PlantGlossaryCategoryAsModifierImport extends CsvImportBase<PlantGlossaryCsvImportConfigurator, PlantGlossaryCsvImportState, File>{
    private static final long serialVersionUID = -5600766240192189822L;
    private static Logger logger = Logger.getLogger(PlantGlossaryCategoryAsModifierImport.class);
    ...
    protected void handleSingleLine(PlantGlossaryCsvImportState importState) {
        ...
        DefinedTerm modifier= DefinedTerm.NewModifierInstance(description, label, null);
        modifier.setUri(URI.create(uri));

        TermVocabulary vocabulary = importState.getModifierVoc();
        vocabulary.addTerm(modifier);

        modifier.addSource(IdentifiableSource.NewInstance(OriginalSourceType.Import, importState.getCitation().getTitle(), null, importState.getCitation(), null));

        getVocabularyService().saveOrUpdate(vocabulary);
        getTermService().saveOrUpdate(modifier);
    }
